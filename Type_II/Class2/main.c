...
void Fp3_mul(Fp3 *ANS,Fp3 *A,Fp3 *B){
    //CVMA
    //AB = (c0, c1, c2)
    //= (a0τ0 + a1τ1 + a2τ2)(b0τ0 + b1τ1 + b2τ2)
    //= (a0b1 + a1b0 + a1b2 + a2b1 – 2a0b0 – 2a1b1 – a2b2) τ0
    //+ (a0b2 + a2b0 + a1b2 + a2b1 – a0b0 – 2a1b1 – 2a2b2) τ1
    //+ (a0b1 + a1b0 + a0b2 + a2b0 – 2a0b0 – a1b1 – 2a2b2) τ2
    
    Fp_mul(&TMP1_FP,&A->x0,&B->x0);
    Fp_mul(&TMP2_FP,&A->x1,&B->x1);
    Fp_mul(&TMP3_FP,&A->x2,&B->x2);
    
    Fp_sub(&TMP7_FP,&A->x0,&A->x1);
    Fp_sub(&TMP4_FP,&B->x1,&B->x0);
    Fp_mul(&TMP4_FP,&TMP4_FP,&TMP7_FP);
    
    Fp_sub(&TMP7_FP,&A->x1,&A->x2);
    Fp_sub(&TMP5_FP,&B->x2,&B->x1);
    Fp_mul(&TMP5_FP,&TMP5_FP,&TMP7_FP);
    
    Fp_sub(&TMP7_FP,&A->x0,&A->x2);
    Fp_sub(&TMP6_FP,&B->x2,&B->x0);
    Fp_mul(&TMP6_FP,&TMP6_FP,&TMP7_FP);
    
    Fp_add(&ANS->x0,&TMP4_FP,&TMP5_FP);
    Fp_sub(&ANS->x0,&ANS->x0,&TMP1_FP);
    Fp_add(&ANS->x1,&TMP5_FP,&TMP6_FP);
    Fp_sub(&ANS->x1,&ANS->x1,&TMP2_FP);
    Fp_add(&ANS->x2,&TMP4_FP,&TMP6_FP);
    Fp_sub(&ANS->x2,&ANS->x2,&TMP3_FP);
}
...
void Fp3_mul_basis(Fp3 *ANS,Fp3 *A){
    ...
    Fp_set(&TMP2_FP,&A->x1);
    Fp_set(&TMP3_FP,&A->x2);
    Fp_add(&TMP4_FP,&TMP2_FP,&TMP2_FP);
    
    //tau2
    Fp_sub(&ANS->x1,&TMP3_FP,&TMP1_FP);
    Fp_sub(&ANS->x1,&ANS->x1,&TMP4_FP);
    //tau3
    Fp_sub(&ANS->x2,&TMP2_FP,&TMP3_FP);
    Fp_sub(&ANS->x2,&ANS->x2,&TMP1_FP);
    Fp_sub(&ANS->x2,&ANS->x2,&TMP1_FP);
    //tau1
    Fp_add(&ANS->x0,&ANS->x1,&ANS->x2);
    Fp_add(&ANS->x0,&ANS->x0,&TMP4_FP);
    Fp_sub(&ANS->x0,&ANS->x0,&TMP1_FP);    
}

void Fp3_sqr(Fp3 *ANS,Fp3 *A){
    Fp_sub(&TMP1_FP,&A->x1,&A->x0);
    
    Fp_add(&TMP2_FP,&TMP1_FP,&A->x2);
    Fp_mul(&TMP2_FP,&TMP2_FP,&TMP2_FP);
    
    Fp_sub(&TMP3_FP,&A->x2,&A->x0);
    Fp_sub(&TMP5_FP,&A->x1,&A->x2);
    Fp_mul(&TMP3_FP,&TMP3_FP,&TMP5_FP);
    
    Fp_mul(&TMP4_FP,&TMP1_FP,&TMP1_FP);
    Fp_mul(&TMP5_FP,&A->x1,&A->x1);
    Fp_add(&TMP4_FP,&TMP4_FP,&TMP5_FP);
    
    Fp_add(&TMP5_FP,&A->x0,&A->x2);
    Fp_mul(&TMP5_FP,&TMP5_FP,&TMP1_FP);
    Fp_add(&TMP5_FP,&TMP5_FP,&TMP3_FP);
    
    Fp_sub(&ANS->x0,&TMP5_FP,&TMP4_FP);
    Fp_add(&ANS->x1,&TMP3_FP,&TMP3_FP);
    Fp_sub(&ANS->x1,&ANS->x1,&TMP4_FP);
    Fp_sub(&ANS->x2,&TMP5_FP,&TMP2_FP);
}
...
void Fp3_inv(Fp3 *ANS,Fp3 *A){
    Fp_sub(&TMP4_FP,&A->x2,&A->x0);
    Fp_sub(&TMP5_FP,&A->x0,&A->x1);
    Fp_sub(&TMP6_FP,&A->x2,&A->x1);
    Fp_mul(&TMP7_FP,&TMP4_FP,&TMP4_FP);
    Fp_mul(&TMP8_FP,&TMP5_FP,&TMP5_FP);
    Fp_mul(&TMP9_FP,&TMP6_FP,&TMP6_FP);
    Fp_mul(&TMP10_FP,&A->x1,&A->x2);
    Fp_mul(&TMP11_FP,&A->x0,&A->x2);
    Fp_mul(&TMP12_FP,&A->x0,&A->x1);
    
    Fp_sub(&TMP1_FP,&TMP7_FP,&TMP10_FP);
    Fp_sub(&TMP2_FP,&TMP8_FP,&TMP11_FP);
    Fp_sub(&TMP3_FP,&TMP9_FP,&TMP12_FP);
    
    Fp_add(&TMP14_FP,&TMP5_FP,&A->x0);
    Fp_mul(&TMP14_FP,&TMP14_FP,&TMP1_FP);
    Fp_add(&TMP13_FP,&TMP5_FP,&TMP6_FP);
    Fp_mul(&TMP13_FP,&TMP13_FP,&TMP2_FP);
    Fp_sub(&TMP14_FP,&TMP13_FP,&TMP14_FP);
    Fp_mul(&TMP13_FP,&TMP6_FP,&TMP3_FP);
    Fp_sub(&TMP14_FP,&TMP14_FP,&TMP13_FP);
    
    Fp_inv(&TMP14_FP,&TMP14_FP);
    Fp_set_neg(&TMP14_FP,&TMP14_FP);
    
    Fp_mul(&ANS->x0,&TMP1_FP,&TMP14_FP);
    Fp_mul(&ANS->x1,&TMP2_FP,&TMP14_FP);
    Fp_mul(&ANS->x2,&TMP3_FP,&TMP14_FP);
}
...
void Fp3_sqrt(Fp3 *ANS,Fp3 *A){//TODO
    Fp3 tmp_A;
    Fp3_init(&tmp_A);
    Fp bar_a0,bar_a1,bar_a2,t0,t1,t2,t3,t4,t5,t6,t7,t8,tmp,s;
    Fp_init(&bar_a0);
    Fp_init(&bar_a1);
    Fp_init(&bar_a2);
    Fp_init(&t0);
    ...
    Fp_mul(&s,&s,&bar_a0);
    Fp_add(&tmp,&t1,&t2);
    Fp_mul(&tmp,&tmp,&bar_a1);
    Fp_sub(&s,&tmp,&s);
    Fp_mul(&tmp,&t2,&bar_a2);
    Fp_sub(&s,&s,&tmp);
    Fp_set_neg(&s,&s);
...
int  Fp3_cmp_one(Fp3 *A){
    Fp3_set_neg(&TMP1_FP3,A);
    if(Fp_cmp_one(&TMP1_FP3.x0)==0 && Fp_cmp_one(&TMP1_FP3.x1)==0 && Fp_cmp_one(&TMP1_FP3.x2)==0){
        return 0;
    }
    return 1;
}
...
void Fp3_frobenius_map_p1(Fp3 *ANS,Fp3 *A){
    Fp3_set(&TMP1_FP3,A);
    Fp_set(&ANS->x0,&TMP1_FP3.x1);
    Fp_set(&ANS->x1,&TMP1_FP3.x2);
    Fp_set(&ANS->x2,&TMP1_FP3.x0);
}

void Fp3_frobenius_map_p2(Fp3 *ANS,Fp3 *A){
    Fp3_set(&TMP1_FP3,A);
    Fp_set(&ANS->x0,&TMP1_FP3.x2);
    Fp_set(&ANS->x1,&TMP1_FP3.x0);
    Fp_set(&ANS->x2,&TMP1_FP3.x1);
}
...
void Fp9_sqrt(Fp9 *ANS,Fp9 *A){//TODO
    ...
    Fp3_set(&tmp_A.x1,&A->x1);
    Fp3_mul_mpz(&tmp_A.x1,&tmp_A.x1,frobenius_constant[f_p3][1].x0.x0);
    Fp3_set_neg(&tmp_A.x1,&tmp_A.x1);
    Fp3_set(&tmp_A.x2,&A->x2);
    Fp3_mul_mpz(&tmp_A.x2,&tmp_A.x2,frobenius_constant[f_p3][2].x0.x0);
    Fp3_set_neg(&tmp_A.x2,&tmp_A.x2);
...
void Fp18_sqr_cyclotomic(Fp18 *ANS,Fp18 *A){
    ...
    Fp3_sub_ui(&ANS->x0.x0,&ANS->x0.x0,1);
    
    Fp9_sub(&ANS->x1,&TMP1_FP9,&ANS->x1);
    Fp9_sub(&ANS->x1,&ANS->x1,&TMP2_FP9);
    Fp3_add_ui(&ANS->x1.x0,&ANS->x1.x0,1);
}
...
void Fp18_frobenius_map_p1(Fp18 *ANS,Fp18 *A){
    //1
    Fp3_frobenius_map_p1(&ANS->x0.x0,&A->x0.x0);
    //beta
    Fp3_frobenius_map_p1(&ANS->x0.x1,&A->x0.x1);
    Fp3_mul(&ANS->x0.x1,&ANS->x0.x1,&frobenius_constant[f_p1][1]);
    //beta^2
    Fp3_frobenius_map_p1(&ANS->x0.x2,&A->x0.x2);
    Fp3_mul(&ANS->x0.x2,&ANS->x0.x2,&frobenius_constant[f_p1][2]);
    //gamma
    Fp3_frobenius_map_p1(&ANS->x1.x0,&A->x1.x0);
    Fp3_mul(&ANS->x1.x0,&ANS->x1.x0,&frobenius_constant[f_p1][3]);
    //betagamma
    Fp3_frobenius_map_p1(&ANS->x1.x1,&A->x1.x1);
    Fp3_set(&TMP1_FP3,&ANS->x1.x1);
    Fp_sub(&ANS->x1.x1.x0,&TMP1_FP3.x1,&TMP1_FP3.x0);
    Fp_add(&ANS->x1.x1.x1,&ANS->x1.x1.x0,&TMP1_FP3.x2);
    Fp_sub(&ANS->x1.x1.x2,&ANS->x1.x1.x1,&TMP1_FP3.x0);
    Fp_add(&ANS->x1.x1.x2,&ANS->x1.x1.x2,&TMP1_FP3.x2);
    
    //beta^2gamma
    Fp3_frobenius_map_p1(&ANS->x1.x2,&A->x1.x2);
    Fp3_mul(&ANS->x1.x2,&ANS->x1.x2,&frobenius_constant[f_p1][5]);
}

void Fp18_frobenius_map_p2(Fp18 *ANS,Fp18 *A){
    //1
    Fp3_frobenius_map_p2(&ANS->x0.x0,&A->x0.x0);
    //beta
    Fp3_frobenius_map_p2(&ANS->x0.x1,&A->x0.x1);
    Fp3_mul(&ANS->x0.x1,&ANS->x0.x1,&frobenius_constant[f_p2][1]);
    //beta^2
    Fp3_frobenius_map_p2(&ANS->x0.x2,&A->x0.x2);
    Fp3_mul(&ANS->x0.x2,&ANS->x0.x2,&frobenius_constant[f_p2][2]);
    //gamma
    Fp3_frobenius_map_p2(&ANS->x1.x0,&A->x1.x0);
    Fp3_mul(&ANS->x1.x0,&ANS->x1.x0,&frobenius_constant[f_p2][3]);
    //betagamma
    Fp3_frobenius_map_p2(&ANS->x1.x1,&A->x1.x1);
    Fp3_set(&TMP1_FP3,&ANS->x1.x1);
    Fp_sub(&ANS->x1.x1.x0,&TMP1_FP3.x1,&TMP1_FP3.x2);
    Fp_sub(&ANS->x1.x1.x2,&TMP1_FP3.x0,&TMP1_FP3.x2);
    Fp_add(&ANS->x1.x1.x1,&ANS->x1.x1.x0,&ANS->x1.x1.x2);
    Fp_sub(&ANS->x1.x1.x2,&ANS->x1.x1.x2,&TMP1_FP3.x2);
    
    //beta^2gamma
    Fp3_frobenius_map_p2(&ANS->x1.x2,&A->x1.x2);
    Fp3_mul(&ANS->x1.x2,&ANS->x1.x2,&frobenius_constant[f_p2][5]);
}
...
void Fp18_frobenius_map_p3(Fp18 *ANS,Fp18 *A){
    ...
    //beta
    Fp3_set(&ANS->x0.x1,&A->x0.x1);
    Fp3_mul_mpz(&ANS->x0.x1,&ANS->x0.x1,frobenius_constant_p3[1].x0);
    //beta^2
    Fp3_set(&ANS->x0.x2,&A->x0.x2);
    Fp3_mul_mpz(&ANS->x0.x2,&ANS->x0.x2,frobenius_constant_p3[2].x0);
    //gamma
    Fp3_set(&ANS->x1.x0,&A->x1.x0);
    Fp3_mul_mpz(&ANS->x1.x0,&ANS->x1.x0,frobenius_constant_p3[3].x0);
    //betagamma
    Fp3_set_neg(&ANS->x1.x1,&A->x1.x1);
    //beta^2gamma
    Fp3_set(&ANS->x1.x2,&A->x1.x2);
    Fp3_mul_mpz(&ANS->x1.x2,&ANS->x1.x2,frobenius_constant_p3[5].x0);
}
...
void Fp18_frobenius_map_p4(Fp18 *ANS,Fp18 *A){
    ...
    //beta
    Fp3_frobenius_map_p1(&ANS->x0.x1,&A->x0.x1);
    Fp3_mul(&ANS->x0.x1,&ANS->x0.x1,&frobenius_constant[f_p4][1]);
    //beta^2
    Fp3_frobenius_map_p1(&ANS->x0.x2,&A->x0.x2);
    Fp3_mul(&ANS->x0.x2,&ANS->x0.x2,&frobenius_constant[f_p4][2]);
    //gamma
    Fp3_frobenius_map_p1(&ANS->x1.x0,&A->x1.x0);
    Fp3_mul(&ANS->x1.x0,&ANS->x1.x0,&frobenius_constant[f_p4][3]);
    //betagamma
    Fp3_frobenius_map_p1(&ANS->x1.x1,&A->x1.x1);
    Fp3_set(&TMP1_FP3,&ANS->x1.x1);
    Fp_sub(&ANS->x1.x1.x0,&TMP1_FP3.x0,&TMP1_FP3.x1);
    Fp_sub(&ANS->x1.x1.x1,&ANS->x1.x1.x0,&TMP1_FP3.x2);
    Fp_add(&ANS->x1.x1.x2,&ANS->x1.x1.x1,&TMP1_FP3.x0);
    Fp_sub(&ANS->x1.x1.x2,&ANS->x1.x1.x2,&TMP1_FP3.x2);
    //beta^2gamma
    Fp3_frobenius_map_p1(&ANS->x1.x2,&A->x1.x2);
    Fp3_mul(&ANS->x1.x2,&ANS->x1.x2,&frobenius_constant[f_p4][5]);
}
...
void Fp18_frobenius_map_p5(Fp18 *ANS,Fp18 *A){
    ...
    //beta
    Fp3_frobenius_map_p2(&ANS->x0.x1,&A->x0.x1);
    Fp3_mul(&ANS->x0.x1,&ANS->x0.x1,&frobenius_constant[f_p5][1]);
    //beta^2
    Fp3_frobenius_map_p2(&ANS->x0.x2,&A->x0.x2);
    Fp3_mul(&ANS->x0.x2,&ANS->x0.x2,&frobenius_constant[f_p5][2]);
    //gamma
    Fp3_frobenius_map_p2(&ANS->x1.x0,&A->x1.x0);
    Fp3_mul(&ANS->x1.x0,&ANS->x1.x0,&frobenius_constant[f_p5][3]);
    //betagamma
    Fp3_frobenius_map_p2(&ANS->x1.x1,&A->x1.x1);
    //Fp3_mul(&ANS->x1.x1,&ANS->x1.x1,&frobenius_constant[f_p5][4]);
    Fp3_set(&TMP1_FP3,&ANS->x1.x1);
    Fp_sub(&ANS->x1.x1.x0,&TMP1_FP3.x2,&TMP1_FP3.x1);
    Fp_sub(&ANS->x1.x1.x2,&TMP1_FP3.x2,&TMP1_FP3.x0);
    Fp_add(&ANS->x1.x1.x1,&ANS->x1.x1.x0,&ANS->x1.x1.x2);
    Fp_add(&ANS->x1.x1.x2,&ANS->x1.x1.x2,&TMP1_FP3.x2);
    //beta^2gamma
    Fp3_frobenius_map_p2(&ANS->x1.x2,&A->x1.x2);
    Fp3_mul(&ANS->x1.x2,&ANS->x1.x2,&frobenius_constant[f_p5][5]);
}
void Fp18_frobenius_map_p6(Fp18 *ANS,Fp18 *A){
    ...
    //beta
    Fp3_set(&ANS->x0.x1,&A->x0.x1);
    Fp3_mul_mpz(&ANS->x0.x1,&ANS->x0.x1,frobenius_constant_p6[1].x0);
    //beta^2
    Fp3_set(&ANS->x0.x2,&A->x0.x2);
    Fp3_mul_mpz(&ANS->x0.x2,&ANS->x0.x2,frobenius_constant_p6[2].x0);
    //gamma
    Fp3_set(&ANS->x1.x0,&A->x1.x0);
    Fp3_mul_mpz(&ANS->x1.x0,&ANS->x1.x0,frobenius_constant_p6[3].x0);
    //betagamma
    Fp3_set(&ANS->x1.x1,&A->x1.x1);
    //beta^2gamma
    Fp3_set(&ANS->x1.x2,&A->x1.x2);
    Fp3_mul_mpz(&ANS->x1.x2,&ANS->x1.x2,frobenius_constant_p6[5].x0);
}
...
void EFp3_rational_point(EFp3 *P){
    ...
    while(1){
        ...
        Fp_sub(&tmp2.x0,&tmp2.x0,&curve_b);
        Fp_sub(&tmp2.x1,&tmp2.x1,&curve_b);
        Fp_sub(&tmp2.x2,&tmp2.x2,&curve_b);
...
void EFp3_skew_frobenius_map_p1(EFp3 *ANS,EFp3 *A){
    //beta
    Fp3_frobenius_map_p1(&ANS->x,&A->x);
    Fp3_mul(&ANS->x,&ANS->x,&frobenius_constant[f_p1][1]);
    //betagamma
    Fp3_frobenius_map_p1(&ANS->y,&A->y);
    Fp3_set(&TMP1_FP3,&ANS->y);
    Fp_sub(&ANS->y.x0,&TMP1_FP3.x1,&TMP1_FP3.x0);
    Fp_add(&ANS->y.x1,&ANS->y.x0,&TMP1_FP3.x2);
    Fp_sub(&ANS->y.x2,&ANS->y.x1,&TMP1_FP3.x0);
    Fp_add(&ANS->y.x2,&ANS->y.x2,&TMP1_FP3.x2);
}

void EFp3_skew_frobenius_map_p2(EFp3 *ANS,EFp3 *A){
    //beta
    Fp3_frobenius_map_p2(&ANS->x,&A->x);
    Fp3_mul(&ANS->x,&ANS->x,&frobenius_constant[f_p2][1]);
    //betagamma
    Fp3_frobenius_map_p2(&ANS->y,&A->y);
    Fp3_set(&TMP1_FP3,&ANS->y);
    Fp_sub(&ANS->y.x0,&TMP1_FP3.x1,&TMP1_FP3.x2);
    Fp_sub(&ANS->y.x2,&TMP1_FP3.x0,&TMP1_FP3.x2);
    Fp_add(&ANS->y.x1,&ANS->y.x0,&ANS->y.x2);
    Fp_sub(&ANS->y.x2,&ANS->y.x2,&TMP1_FP3.x2);
}


void EFp3_skew_frobenius_map_p3(EFp3 *ANS,EFp3 *A){
    //beta
    Fp3_frobenius_map_p3(&ANS->x,&A->x);
    Fp3_mul_mpz(&ANS->x,&ANS->x,frobenius_constant_p3[1].x0);
    //betagamma
    Fp3_set_neg(&ANS->y,&A->y);
}
...
void EFp9_rational_point(EFp9 *P){
    ...
    while(1){
        ...
        Fp_sub(&tmp2.x0.x0,&tmp2.x0.x0,&curve_b);
        Fp_sub(&tmp2.x0.x1,&tmp2.x0.x1,&curve_b);
        Fp_sub(&tmp2.x0.x2,&tmp2.x0.x2,&curve_b);
...
void EFp18_rational_point(EFp18 *P){
    ...
    while(1){
        ...
        Fp_sub(&tmp2.x0.x0.x0,&tmp2.x0.x0.x0,&curve_b);
        Fp_sub(&tmp2.x0.x0.x1,&tmp2.x0.x0.x1,&curve_b);
        Fp_sub(&tmp2.x0.x0.x2,&tmp2.x0.x0.x2,&curve_b);
...
void EFp_to_EFp18(EFp18 *ANS,EFp *A){
	...
	Fp_set(&ANS->x.x0.x0.x1,&ANS->x.x0.x0.x0);
	Fp_set(&ANS->x.x0.x0.x2,&ANS->x.x0.x0.x0);
	Fp18_set_ui(&ANS->y,0);
	Fp_set_neg(&ANS->y.x0.x0.x0,&A->y);
	Fp_set(&ANS->y.x0.x0.x1,&ANS->y.x0.x0.x0);
	Fp_set(&ANS->y.x0.x0.x2,&ANS->y.x0.x0.x0);
	ANS->infinity=A->infinity;
}
...
void Pseudo_12_sparse_mul(Fp18 *ANS,Fp18 *A,Fp18 *B){
    //A= f0 + f1θ^2 + f2θ^4 + f3θ　+ f4θ^3 + f5θ^5
	//B= 1                  +  aθ +  bθ^3
	// x0.x0  x0.x1  x0.x2  x1.x0   x1.x1   x1.x2
	Fp3_mul(&TMP1_FP3,&A->x0.x0,&B->x1.x0);
	Fp3_mul(&TMP2_FP3,&A->x0.x1,&B->x1.x1);
	Fp3_add(&TMP3_FP3,&A->x0.x0,&A->x0.x1);
	Fp3_add(&TMP4_FP3,&B->x1.x0,&B->x1.x1);
	Fp3_mul(&TMP3_FP3,&TMP3_FP3,&TMP4_FP3);
	Fp3_sub(&TMP3_FP3,&TMP3_FP3,&TMP1_FP3);
	Fp3_sub(&TMP3_FP3,&TMP3_FP3,&TMP2_FP3);
	Fp3_add(&TMP1_FP18.x1.x1,&TMP3_FP3,&A->x1.x1);
	Fp3_add(&TMP1_FP3,&TMP1_FP3,&A->x1.x0);
	Fp3_mul(&TMP3_FP3,&A->x0.x2,&B->x1.x1);
	Fp3_mul_basis(&TMP3_FP3,&TMP3_FP3);
	Fp3_add(&TMP1_FP18.x1.x0,&TMP1_FP3,&TMP3_FP3);
	Fp3_add(&TMP1_FP3,&TMP2_FP3,&A->x1.x2);
	Fp3_mul(&TMP2_FP3,&A->x0.x2,&B->x1.x0);
	Fp3_add(&TMP1_FP18.x1.x2,&TMP1_FP3,&TMP2_FP3);
	Fp3_mul(&TMP1_FP3,&A->x1.x0,&B->x1.x0);
	Fp3_mul(&TMP2_FP3,&A->x1.x1,&B->x1.x1);
	Fp3_add(&TMP3_FP3,&A->x1.x0,&A->x1.x1);
	Fp3_mul(&TMP3_FP3,&TMP3_FP3,&TMP4_FP3);
	Fp3_sub(&TMP3_FP3,&TMP3_FP3,&TMP1_FP3);
	Fp3_sub(&TMP3_FP3,&TMP3_FP3,&TMP2_FP3);
	Fp3_add(&TMP1_FP18.x0.x2,&TMP3_FP3,&A->x0.x2);
	Fp3_add(&TMP1_FP3,&TMP1_FP3,&A->x0.x1);
	Fp3_mul(&TMP3_FP3,&A->x1.x2,&B->x1.x1);
	Fp3_mul_basis(&TMP3_FP3,&TMP3_FP3);
	Fp3_add(&TMP1_FP18.x0.x1,&TMP1_FP3,&TMP3_FP3);
	Fp3_mul(&TMP1_FP3,&A->x1.x2,&B->x1.x0);
	Fp3_add(&TMP1_FP3,&TMP1_FP3,&TMP2_FP3);
	Fp3_mul_basis(&TMP1_FP3,&TMP1_FP3);
	Fp3_add(&TMP1_FP18.x0.x0,&TMP1_FP3,&A->x0.x0);
...
void ff_ltt(Fp18 *f,EFp3 *T,EFp *P,Fp *L){
	...
	Fp3_set_neg(&TMP2_FP18.x1.x0,&TMP3_FP3);
...
void Fp18_G3_EXP_6split(Fp18 *ANS,Fp18 *A,mpz_t scalar){
	...
	//set table
	Fp3_set(&table0to2[0].x0.x0,&ONE);              //000
	...
	Fp3_set(&table3to5[0].x0.x0,&ONE);            //000
...
void init_parameters(){
    ...
    for(i=0; i<18; i++){
        for(j=0; j<6; j++){
            Fp3_init(&frobenius_constant[i][j]);
        }
    }
    
    for(i=0; i<6; i++){
        Fp_init(&frobenius_constant_p3[i]);
        Fp_init(&frobenius_constant_p6[i]);
    }
...
void KSS18_print_parameters(){
    ...
    gmp_printf("\nelliptic curve\n");
    gmp_printf("E:y^2=x^3-7\n");
    gmp_printf("twisted curve\n");
    gmp_printf("E':y^2=x^3-7*alpha^{-1}\n");
    
    gmp_printf("\nmodulo polynomial\n");
    gmp_printf("Fp3 = Fp[alpha]/Phi7(alpha)\n");
    gmp_printf("Fp9 = Fp3[beta]/(beta^3-(tau1-2))\n");
    gmp_printf("Fp18= Fp9[gamma]/(gamma^2-beta)\n");
    gmp_printf("tau1=alpha+alpha^6\ntau2=alpha^2+alpha^5\ntau3=alpha^3+alpha^4\n");
}
...
void KSS18_clear(){
    ...
    for(i=0; i<18; i++){
        for(j=0; j<6; j++){
            Fp3_clear(&frobenius_constant[i][j]);
        }
    }
    
    for(i=0; i<6; i++){
        Fp_clear(&frobenius_constant_p3[i]);
        Fp_clear(&frobenius_constant_p6[i]);
    }
    
    Fp3_clear(&Tau1_2);
    Fp3_clear(&Tau1_2_inv);
    Fp3_clear(&ONE);
...
void set_basis(){
    Fp_set_ui(&Tau1_2.x0,1);
    Fp3_add_ui(&Tau1_2,&Tau1_2,2);
}
...
void set_frobenius_constant(){
	...
	for(i=0; i<18; i++){
	    ...
	    mpz_tdiv_q_ui(exp,exp,3);
	    Fp3_pow(&tmp_beta,&Tau1_2,exp);
	    //gamma
	    mpz_tdiv_q_ui(exp,exp,2);
	    Fp3_pow(&tmp_gamma,&Tau1_2,exp);
	    Fp3_set(&frobenius_constant[i][0],&ONE);
	    Fp3_set(&frobenius_constant[i][1],&tmp_beta);
	    Fp3_sqr(&frobenius_constant[i][2],&tmp_beta);
	    Fp3_set(&frobenius_constant[i][3],&tmp_gamma);
	    Fp3_mul(&frobenius_constant[i][4],&tmp_gamma,&frobenius_constant[i][1]);
	    Fp3_mul(&frobenius_constant[i][5],&tmp_gamma,&frobenius_constant[i][2]);
	}
	
	for(i=0; i<6; i++){
	    Fp_set_neg(&frobenius_constant_p3[i],&frobenius_constant[f_p3][i].x0);
	    Fp_set_neg(&frobenius_constant_p6[i],&frobenius_constant[f_p6][i].x0);
	}
...
void test_Frobenius_map(){
    ...
    Fp18_frobenius_map_p6(&test2,&tmp_Fp18);
    Fp18_printf(&test2,"");
    printf("\n\n");
