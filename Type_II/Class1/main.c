...
void Fp3_inv_basis(Fp3 *ANS,Fp3 *A){
    //Fp3_mul(ANS,A,&Tau1_2_inv);
    Fp3_set(&TMP1_FP3,A);
    
    Fp_add(&TMP1_FP,&TMP1_FP3.x1,&TMP1_FP3.x2);
    Fp_set_neg(&TMP1_FP,&TMP1_FP);
    Fp_add(&TMP2_FP,&TMP1_FP,&TMP1_FP);
    Fp_add(&TMP2_FP,&TMP2_FP,&TMP2_FP);
    Fp_add(&TMP3_FP,&TMP1_FP3.x1,&TMP1_FP3.x1);
    Fp_add(&TMP3_FP,&TMP3_FP,&TMP1_FP3.x1);
    Fp_add(&TMP3_FP,&TMP3_FP,&TMP3_FP);
    Fp_add(&TMP3_FP,&TMP3_FP,&TMP1_FP3.x1);
    
    Fp_sub(&ANS->x0,&TMP1_FP,&TMP1_FP3.x0);
    Fp_add(&ANS->x1,&TMP2_FP,&TMP1_FP3.x0);
    Fp_add(&ANS->x1,&ANS->x1,&TMP1_FP3.x0);
    Fp_add(&ANS->x1,&ANS->x1,&TMP1_FP3.x0);
    Fp_add(&ANS->x2,&ANS->x1,&ANS->x1);
    Fp_add(&ANS->x2,&ANS->x2,&ANS->x0);
    Fp_add(&ANS->x2,&ANS->x2,&TMP3_FP);
    
    Fp3_mul_mpz(ANS,ANS,seven_inv.x0);
}
...
void EFp3_skew_frobenius_map_p1(EFp3 *ANS,EFp3 *A){
    ...
    Fp3_frobenius_map_p1(&ANS->x,&ANS->x);
    Fp3_mul(&ANS->x,&ANS->x,&frobenius_constant[f_p1][2]);
    Fp3_mul_basis(&ANS->x,&ANS->x);
    //betagamma
    Fp3_inv_basis(&ANS->y,&A->y);
    Fp3_frobenius_map_p1(&ANS->y,&ANS->y);
...
void EFp3_skew_frobenius_map_p2(EFp3 *ANS,EFp3 *A){
    //beta
    Fp3_inv_basis(&ANS->x,&A->x);
    Fp3_frobenius_map_p2(&ANS->x,&ANS->x);
    Fp3_mul(&ANS->x,&ANS->x,&frobenius_constant[f_p2][2]);
    Fp3_mul_basis(&ANS->x,&ANS->x);
    //betagamma
    Fp3_inv_basis(&ANS->y,&A->y);
    Fp3_frobenius_map_p2(&ANS->y,&ANS->y);
    Fp3_set(&TMP1_FP3,&ANS->y);
...
void EFp3_skew_frobenius_map_p3(EFp3 *ANS,EFp3 *A){
    //beta
    //Fp3_inv_basis(&ANS->x,&A->x);
    Fp3_mul_mpz(&ANS->x,&A->x,frobenius_constant[f_p3][2].x0.x0);
    Fp3_set_neg(&ANS->x,&ANS->x);
    //Fp3_mul_basis(&ANS->x,&ANS->x);
    //betagamma
    Fp3_set_neg(&ANS->y,&A->y);
}
...
void KSS18_init(){
    ...
    if(generate_prime()==1 && generate_order()==1){
        ...
        Fp3_set_ui(&ONE,1);
        Fp3_set_neg(&ONE,&ONE);
        Fp_set_ui(&seven_inv,7);
        Fp_inv(&seven_inv,&seven_inv);
    }else{
        KSS18_clear();
        printf("error : prime\nexit\n");
        exit(1);
...
void KSS18_print_parameters(){
    ...
    gmp_printf("\nelliptic curve\n");
    gmp_printf("E:y^2=x^3-7^5\n");
    gmp_printf("twisted curve\n");
    gmp_printf("E':y^2=x^3-7^5*alpha\n");
