...
void Fp18_frobenius_map_p2(Fp18 *ANS,Fp18 *A){
    ...
    //beta^2gammaFp18_frobenius_map_p3
...
void EFp3_skew_frobenius_map_p1(EFp3 *ANS,EFp3 *A){
    //beta
    Fp_mul(&ANS->x.x0,&A->x.x0,&frobenius_constant[f_p1][3]);
    Fp_mul(&ANS->x.x1,&A->x.x1,&frobenius_constant[f_p1][4]);
    Fp_mul(&ANS->x.x2,&A->x.x2,&frobenius_constant[f_p1][5]);
    ...
    //betagamma
    Fp_mul(&ANS->y.x0,&A->y.x0,&frobenius_constant[f_p1][12]);
    Fp_set_neg(&ANS->y.x1,&A->y.x1);
    Fp_mul(&ANS->y.x2,&A->y.x2,&frobenius_constant[f_p1][14]);
}

void EFp3_skew_frobenius_map_p2(EFp3 *ANS,EFp3 *A){
    Fp_mul_mpz(&ANS->x.x0,&A->x.x0,frobenius_constant[f_p2][3].x0);
    Fp_mul_mpz(&ANS->x.x1,&A->x.x1,frobenius_constant[f_p2][4].x0);
    Fp_mul_mpz(&ANS->x.x2,&A->x.x2,frobenius_constant[f_p2][5].x0);
    Fp3_set(&TMP1_FP3,&ANS->x);
    Fp_add(&ANS->x.x0,&TMP1_FP3.x1,&TMP1_FP3.x1);
    Fp_add(&ANS->x.x1,&TMP1_FP3.x2,&TMP1_FP3.x2);
    Fp_set(&ANS->x.x2,&TMP1_FP3.x0);
    
    Fp_mul_mpz(&ANS->y.x0,&A->y.x0,frobenius_constant[f_p2][12].x0);
    Fp_mul_mpz(&ANS->y.x1,&A->y.x1,frobenius_constant[f_p2][13].x0);
    Fp_mul_mpz(&ANS->y.x2,&A->y.x2,frobenius_constant[f_p2][14].x0);
}

void EFp3_skew_frobenius_map_p3(EFp3 *ANS,EFp3 *A){
    Fp3_mul_mpz(&ANS->x,&A->x,frobenius_constant[f_p3][3].x0);
    Fp3_set_neg(&ANS->y,&A->y);
}
...
void KSS18_print_parameters(){
    ...
    gmp_printf("E':y^2=x^3+2*alpha^{-1}\n");
...
void set_frobenius_constant(){
    ...
    Fp3 tmp_frobenius_constant[6],tmp_beta,tmp_gamma;
    mpz_t tmp_p[18],exp;
    //init
    Fp_init(&tmp_alpha1);
    Fp_init(&tmp_alpha2);
...
void test_Frobenius_map(){
    ...
    Fp18_frobenius_map_p1(&test2,&tmp_Fp18);
    Fp18_printf(&test2,"");
    printf("\n\n");
...
void test_skew_frobenius_map(){
    ...
    Fp18_frobenius_map_p2(&test1.x,&Q.x);
    Fp18_frobenius_map_p2(&test1.y,&Q.y);
    EFp18_printf(&test1,"");
    printf("\n\n");
    
    EFp3_skew_frobenius_map_p2(&twisted_Q,&twisted_Q);
